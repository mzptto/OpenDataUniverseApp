// Fixed OSDU Response Parser
// Handles the actual structure from OSDU GraphQL responses
...
const parseOsduItem = (rawItem) => {
  try {
    ...
    let parsedItem = firstParse;
    if (typeof firstParse === 'string') {
      parsedItem = JSON.parse(firstParse);
    }
    
    // Step 3: Extract the actual OSDU record from the 'source' wrapper
    if (parsedItem.source) {
      return {
        ...parsedItem.source,
        score: parsedItem.score // Keep the search score
      };
    }
...
const parsed = parseOsduItem(testRawItem);
console.log('Parsed OSDU record:', parsed);
